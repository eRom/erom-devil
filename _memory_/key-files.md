...
- `glm.md` — claude -p ollama ; inputs EMBARQUÉS (run hermétique) entre
  marqueurs `=== BEGIN LABEL ===` ; parse .result + strip fences.
- `deepseek.md` — jumeau sed de glm.md (modèle d'abord). À régénérer
  par sed si glm change + contrôle post-gen (0 « glm » résiduel).
- `opus.md` — claude -p (Opus 4.8 xHigh) ; même contrat de transport ;
  hors swarms, dispo en unitaire (`/erom-devil:<skill> opus`).
- `kimi.md` — jumeau sed de glm.md (kimi-k3:cloud[1m]) ; hors
  swarms comme opus, juge indépendant en unitaire (`/erom-devil:<skill> kimi`).
...
## Skills (skills/)
- `spec/SKILL.md` — unitaire spec ; 2 inputs ; boucle correction max 2.
- `spec-swarm/SKILL.md` — tribunal ; tri convergence PUIS sévérité.
- `brain/SKILL.md` — unitaire brain ; 1 input ; 0 question = fin sans
  Q&A ; Q&A amende le brainstorming avec les réponses de Romain uniquement.
- `brain-swarm/SKILL.md` — 3 voix parallèles ; consolidation par
  l'orchestrateur (LLM natif) ; tri criticité PUIS convergence (inverse de
  spec-swarm, assumé) ; écartées → non-buts possibles.
- `code/SKILL.md` — unitaire code ; résolution target (PR/range/
...
- `code-swarm/SKILL.md` — tribunal code (opus et kimi exclus) ; ancrage par
...
- `/Users/recarnot/dev/erom-marketplace/.claude-plugin/marketplace.json` —
  metadata 0.18.3, entrée `erom-devil` version 0.6.0 (URL github erom-devil).
  Lu le 2026-08-18 : la 0.7.0 n'y est PAS encore, la livraison la portera.
